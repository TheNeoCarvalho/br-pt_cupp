...
def print_to_file(filename, unique_list_finished):
	...
	for line in f:
		lines += 1
	f.close()
	print "[+] Dicionario salvo em \033[1;31m"+filename+"\033[1;m, contem \033[1;31m"+str(lines)+" palavras.\033[1;m"
	print "[+] Agora e so usar a lista \033[1;31m"+filename+"\033[1;m e correr pro abraco! Boa Sorte!"
...
if len(sys.argv) < 2 or sys.argv[1] == '-h':
	...
	print "		            [ Manoel Carvalho | manoelmecat@gmail.com ]\r\n\r\n"

	print "	[ Opcoes ]\r\n"
	print "	-h	Voce pode dar uma olhada aqui! :)"
	print " 		Para mais ajuda, acesse: docs/README"
	print "		Configuracao Global no arquivo: cupp.cfg\n"

	print "	-i	Perguntas interativas para perfis de senha de usuario.\r\n"

	print "	-w	Use esta opcao para melhorar o dicionario existente.\r\n"

	print "	-l	Faca o download de grandes listas de palavras do repositorio.\r\n"
	print "	-a	Analise os nomes de usuario e senhas padrao diretamente do Alecto DB."
	print "		Projeto Alecto usa bancos de dados purificados de Phenoelit e CIRT"
	print "		Que se fundiram e melhoraram.\r\n"
	print "	-v	Versao do programa\r\n"
	exit()

elif sys.argv[1] == '-v':
	print "\r\n	\033[1;31m[ cupp.py ]  v3.1.0-alpha\033[1;m\r\n"
	print "	* Hacked up by j0rgan - j0rgan@remote-exploit.org"
	print "	* Traduzido por N30\r\n"
	print "	* http://www.remote-exploit.org\r\n"
	print "	De uma olhada ./README.md arquivo com mais informacoes sobre o programa.\r\n"
	exit()

elif sys.argv[1] == '-w':
	if len(sys.argv) < 3:
		print "\r\n[Usar]:	"+sys.argv[0]+"  -w  [FILENAME]\r\n"
		exit()
	...
	print "[+] Gerando lista e removendo palavras duplicadas..."
	...
	print "\r\n[+] Insira algumas informacoes sobre a sua vitima para criar seu dicionario"
	print "[+] Se voce nao sabe de todas as informacoes, apenas pressione enter nas perguntas ;)\r\n"

# We need some informations first!

	name = raw_input("> Vitima) Primeiro Nome: ").lower()
	while len(name) == 0 or name == " " or name == "  " or name == "   ":
		print "\r\n[-] You must enter a name at least!"
		name = raw_input("> Vitima) Nome: ").lower()
	name = str(name)

	surname = raw_input("> Vitima) Sobrenome: ").lower()
	nick = raw_input("> Vitima) Apelido: ").lower()
	birthdate = raw_input("> Vitima) Data de Aniversario (DDMMYYYY): ")
	while len(birthdate) != 0 and len(birthdate) != 8:
		print "\r\n[-] Deve conter 8 digitos!"
		birthdate = raw_input("> Vitima) Data de Aniversario (DDMMYYYY): ")
	birthdate = str(birthdate)

	print "\r\n"

	wife = raw_input("> Parentes) nome: ").lower()
	wifen = raw_input("> Parentes) Apelido: ").lower()
	wifeb = raw_input("> Parentes) Data de Aniversario (DDMMYYYY): ")
	while len(wifeb) != 0 and len(wifeb) != 8:
		print "\r\n[-] Deve conter 8 digitos!"
		wifeb = raw_input("> Data de Aniversario (DDMMYYYY): ")
	wifeb = str(wifeb)
	print "\r\n"

	kid = raw_input("> Nome do filho: ").lower()
	kidn = raw_input("> Apelido: ").lower()
	kidb = raw_input("> Data de Aniversario (DDMMYYYY): ")
	...
	pet = raw_input("> Animal de estimacao: ").lower()
	company = raw_input("> Empresa que trabalha: ").lower()
	print "\r\n"

	words = ['']
	words1 = raw_input("> Voce gostaria de adicionar palavras chaves sobre a vitima? Y/[N]: ").lower()
	words2 = ""
	if words1 == "y":
		words2 = raw_input("> Por favor entre com as palavras, separadas com virgula. [ex. hacker,juice,black], espacos serao removidos: ").replace(" ","")
	words = words2.split(",")

	spechars = ['']
	spechars1 = raw_input("> Voce gostaria de adicionar caracteres especiais no inicio e fim das palavras? Y/[N]: ").lower()
	if spechars1 == "y":
		for spec1 in chars:
			spechars.append(spec1)
			for spec2 in chars:
	...
	randnum = raw_input("> Voce gostaria de adicionar numeros aleatorios no inicio e fim das palavras? Y/[N]:").lower()
	leetmode = raw_input("> Modo Leet? (ex. leet = 1337) Y/[N]: ").lower()
...
# Birthdays first
	...
	print "[+] Criando lista e removendo palavras duplicadas..."
	...
	print "	\r\n	Escolha a opcao que pretende baixar::\r\n"
	...
	print "	\r\n	Os arquivos seram  baixados para o repositorio "+ftpname
	print "	\r\n	Dica: Depois de baixar a lista de palavras, voce pode aperfeicoa-la com a opcao -w\r\n"

	filedown = raw_input("> Entre com um numero: ")
	filedown.isdigit()
	...
	print "\r\n[Usar]:	"+sys.argv[0] +"  [OPTIONS] \r\n"
	print "[Ajuda]:		"+sys.argv[0] +"  -h\r\n"
	exit()
